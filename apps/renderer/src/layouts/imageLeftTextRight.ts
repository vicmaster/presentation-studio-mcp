...
export const imageLeftTextRightLayout: LayoutRenderer = (slide, spec, ctx) => {
  ...
  const [imageCol, textCol] = splitColumns(contentArea, 2, ctx.spacing.gap);
  const imageRect = {
    x: imageCol!.x,
    y: imageCol!.y,
    w: imageCol!.w,
  ...
  if (image) {
    ...
    warnings.push(`image-left-text-right on slide "${spec.id}" has no image.`);
  }
  ...
  if (spec.kicker) {
    drawKicker(
      slide,
      ctx,
      { x: textCol!.x, y: textCol!.y, w: textCol!.w, h: 0.3 },
      spec.kicker,
    );
  }
